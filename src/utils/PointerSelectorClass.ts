...
export class PointerSelector {
  ...
  constructor(
    ...
    if (!this.createPointerSelector(this._clickHandler))
      throw new Error("Failed to create pointer selector, already exists");
  ...
  private readonly clickHandlerResolver = (e: MouseEvent): void => {
    ...
    if (t === document.firstElementChild) {
      console.debug(
        "Target is document html, allowing retry Source Event: ",
        e
      );
      return;
    }
    ...
    try {
      ...
      if (remove) {
        document
          ...
          ?.removeEventListener("click", this.clickHandlerResolver);
        document
          .getElementById(this._pointerSelectorClassName)
          ?.removeEventListener(
            "mouseover",
            this.pointerSelectorMouseEventForwarder
          );
  ...
  private readonly adjustPointerSelector = (target: HTMLElement): void => {
    ...
    if (!this._pointerSelector)
      throw new Error("Pointer selector does not exist");
  ...
  private readonly getTargetFromCachedCoords: (
    sourceEvent: MouseEvent
  ) => HTMLElement = (sourceEvent) => {
    if (!this._pointerSelector)
      throw new Error("Pointer selector does not exist");
  ...
  private readonly createPointerSelector = (
    clickHandler: ((e: HTMLElement) => boolean) | undefined
  ): boolean => {
    if (this._pointerSelector) {
    ...
    if (typeof clickHandler === "function") {
      ...
      this._pointerSelector.addEventListener(
        "click",
        this.clickHandlerResolver,
        {
          passive: false,
          once: false,
          capture: true,
  ...
  private readonly pointerSelectorMouseEventForwarder = (
    e: MouseEvent
  ): void => {
