...
function IdentifyLabelTargets(x: HTMLLabelElement): HTMLElement | undefined {
  ...
  if (implicitlyLabelledEl.length > 1) {
    console.warn(
      "label has multiple implicitly labelled elements, ",
      x,
      implicitlyLabelledEl
    );
...
interface LabelledElement {
  name: string;
  labellingMethod: Labelling;
  target: HTMLElement;
}
function IdentifyExplicitNames(target: HTMLElement): boolean {
  console.log("target: ", target);
  console.log("name: ", getName(target));
  const hasAriaLabel: LabelledElement[] = Array.from(
    target.querySelectorAll("[aria-label]")
  ).map((x) => ({
    name: x.getAttribute("aria-label") as string,
    labellingMethod: Labelling.ariaLabel,
    target: x as HTMLElement,
  }));
  const hasAriaLabeledBy: LabelledElement[] = Array.from(
    target.querySelectorAll("[aria-labelledby],[aria-labeledby]")
  )
    .concat(target)
    .map((x) => ({
      name: getName(x),
  ...
  if (
    ...
    target.getAttribute("aria-labeledby")
  ) {
  ...
  const hasExternalLabel: LabelledElement[] = Array.from(
    target.querySelectorAll("label")
  )
    .map(IdentifyLabelTargets)
    .filter((x) => x !== undefined)
    .map((x) => ({
      name: getName(x as HTMLElement),
